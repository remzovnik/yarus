...
// const utm = "utm_source=yarus.ru&utm_medium=site&utm_campaign=app_page";
const googleUrl = `https://redirect.appmetrica.yandex.com/serve/100012788742674373`;
const appleUrl = `https://redirect.appmetrica.yandex.com/serve/532358357852453612`;
...
onBeforeMount(() => {
  setTimeout(() => {
    ...
    if (route.path === ERouteAlias.APP2) {
      try {
        link.setAttribute("href", "https://redirect.appmetrica.yandex.com/serve/243819262421057430");
      } catch (err) {
        console.log(err);
      }
    }

    if (route.path === ERouteAlias.APP3) {
      try {
        link.setAttribute("href", "https://redirect.appmetrica.yandex.com/serve/171786449656528394");
      } catch (err) {
        console.log(err);
      }
    }

    if (route.path === ERouteAlias.APP4) {
      try {
        link.setAttribute("href", "https://redirect.appmetrica.yandex.com/serve/1108545418381237305");
      } catch (err) {
        console.log(err);
      }
    }
...
onMounted(async () => {
  ...
  setTimeout(() => {
    if (isIOS && isMobile) {
      ...
      link.setAttribute("href", appleUrl);
      link.click();
      link.remove();
    }
